...
For detailed architecture principles, refer to [AGENTS.md](AGENTS.md).  

## Critical Architecture Decisions

### Route-Centric Data Management ✅ **IMPLEMENTED**
- **✅ CONFIRMED**: Routes own their waypoints (strong data relationships)  
- **✅ CONFIRMED**: RouteCache mimics AirspaceCache with flat buffer storage (see RouteCache.kt)  
- **✅ CONFIRMED**: Hilbert spatial indexing for efficient route queries (see RouteCacheManager.kt)  
- **✅ CONFIRMED**: 10-route limit with distance-based spatial filtering  
- **✅ CONFIRMED**: Memory-mapped I/O for zero-copy route loading (see RouteCache.kt:100-150)  
...
Refer to [AGENTS.md](AGENTS.md) for detailed Redux migration strategy and anti-patterns.  
...
**Architecture**: RouteCache with flat buffer storage and spatial indexing (see RouteCache.kt)  
...
**Deliverables**:  
- [ ] **RouteCache**: Flat buffer storage with Hilbert indexing  
...
- RouteState.kt, RouteActions.kt, RouteReducers.kt (new files)  
- Redux bridge to sync with RouteCache (see RouteStore.kt:50-100)  
- RouteOverlayManager.kt extending BaseOverlayManager  

**Deliverables**:  
- [ ] Redux state management implementation  
- [ ] RouteOverlayManager for map visualization (see RouteOverlayManager.kt)  
...
- WeatherRouter integration (see WeatherRouter.kt)  
...
### Waypoint Operations
- **ADD Waypoint**: MapViewContainer → Route.addWaypoint() → RouteCache.persistRoute() → RouteOverlayManager.updateVisuals()  
- **MODIFY Waypoint (Drag)**: MapViewContainer → Route.updateWaypoint() → RouteCache.updateSpatialIndex() → RouteOverlayManager.redrawRoute()  
- **DELETE Waypoint**: MapViewContainer → Route.removeWaypoint() → RouteCache.persistRoute() → RouteOverlayManager.removeMarker()  
- **QUERY Routes**: MapViewContainer → RouteCache.queryNearbyRoutes() → Spatial Filter → UI Display  

### State Management
- **Current: RouteCache (Phase 1)**: Route Operations → RouteCache → Flat Buffer Storage → Spatial Index  
- **Future: Redux + RouteCache (Phase 2)**: Route Operations → Redux Actions → RouteCache Sync → StateFlow Updates  
...
- **[AGENTS.md](AGENTS.md)**: Updated route-centric architecture, Redux migration strategy, and performance guidelines.  
- **[ARCHITECTURE_DECISIONS.md](docs/ARCHITECTURE_DECISIONS.md)**: System architecture and caching patterns.  
- **[AVIATION_SAFETY.md](docs/AVIATION_SAFETY.md)**: Safety standards and performance requirements.  
- **[PERFORMANCE_GUIDELINES.md](docs/PERFORMANCE_GUIDELINES.md)**: Performance targets and monitoring.  
